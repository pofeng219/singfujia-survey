...
export const validateForm = (d: SurveyData, type: SurveyType): ValidationError[] => {
    ...
    if (type === 'factory') {
        ...
        v.requireIf(d.propertyType === '其他', d.propertyTypeOther, "section-propertyType", "本物件型態與現況 (請填寫「其他」說明)", 1);
    }
    ...
    // Step 1: Access
    const accessTitle = (type === 'factory' || type === 'house') ? "本物件型態與現況" : "本物件現況";
    ...
    if (d.access === '不可進入') {
        ...
        v.requireIf(d.accessType?.includes('其他') || false, d.accessOther, "section-access", `${accessTitle} (請填寫不可進入之「其他」詳細內容)`, 1);
    }
    ...
    const validateParkingPart = (baseNum: number, startStep: number, sMain: string) => {
        ...
        v.check(motoEmpty && !d.q10_hasMotoUsageOther, sMain, `${qInfo}：機車車位-請至少勾選一項使用情況`, startStep);
        ...
        if (!d.q10_noParking) {
            ...
            if (!isOtherPt) {
                ...
                v.check(carUsageEmpty && !d.q10_hasCarUsageOther, sMain, `${qInfo}：汽車使用情況未填寫`, startStep);
            ...
            // Abnormal
            if (d.q11_hasIssue === '是') {
                v.requireCheckboxGroup(true, d.q11_items, d.q11_hasOther, sMain, `${qInfo}：請勾選異常情況`, startStep);
    ...
    if (type === 'house') {
        const s2 = 2;
        v.require(d.q1_hasExt, "section-q1", "1. 增建情況未填寫", s2);
        ...
        v.require(d.q5_hasTilt, "section-q4", "4. 傾斜情況未填寫", s2);
        ...
        v.require(d.water_booster, "section-q7", "5. 是否設置用戶加壓受水設備未填寫", s2);
        v.requireIf(d.water_booster === '其他', d.water_booster_desc, "section-q7", "5. 請填寫加壓設備「其他」說明", s2);

        v.require(d.q7_hasIssue, "section-q7", "5. 電、水與瓦斯異常情況未填寫", s2);
        if (d.q7_hasIssue === '是') {
        ...
        const s3 = 3;
        v.require(d.publicFacilities, "section-publicFacilities", "公共設施情況未填寫", s3);
        v.requireIf(d.publicFacilities === '無法進入', d.publicFacilitiesReason, "section-publicFacilities", "請填寫無法進入公設之原因", s3);

        v.require(d.q8_stairIssue, "section-q8", "6. 梯間/地下室異常未填寫", s3);
        if (d.q8_stairIssue === '是') {
            const stairEmpty = (d.q8_stairItems?.length || 0) === 0;
            v.check(stairEmpty && !d.q8_stairOther, "section-q8", "6. 請勾選或填寫異常情況", s3);
        }

        v.require(d.q9_hasIssue, "section-q9", "7. 社區須注意設施未填寫", s3);
        if (d.q9_hasIssue === '是') {
            v.requireCheckboxGroup(true, d.q9_items, d.q9_hasOther, "section-q9", "7. 請勾選或填寫設施", s3);
            v.requireOther(d.q9_hasOther, d.q9_otherDesc, "section-q9", "7. 請填寫「其他」設施說明", s3);
            // Solar PV Maintenance Validation
            if (d.q9_items?.includes("太陽能光電發電設備")) {
                v.require(d.q9_solar_maintenance, "section-q9", "7. 請選擇太陽能光電發電設備維護方式", s3);
            }
        }

        validateParkingPart(8, s3, 'section-house-parking-main');

        const s4 = 4;
        v.require(d.q14_access, "section-q14", "9. 本案進出情況未填寫", s4);
        if (d.q14_access === '正常') {
             v.require(d.q14_ownership, "section-q14", "9. 請選擇通行權屬 (公有/私人)", s4);
             v.require(d.q14_roadMaterial, "section-q14", "9. 請選擇路面材質", s4);
             v.requireIf(d.q14_roadMaterial === '其他', d.q14_roadMaterialOther, "section-q14", "9. 請填寫路面材質說明", s4);
        } else if (d.q14_access === '異常') {
             v.require(d.q14_abnormalDesc, "section-q14", "9. 請填寫異常描述", s4);
        }
        
        const noEnv = d.q16_noFacilities === false && (d.q16_items?.length || 0) === 0 && !d.q16_hasOther;
        v.check(noEnv, "section-q16", "10. 環境設施未勾選 (若無請勾選無重要環境設施)", s4);
        v.requireOther(d.q16_hasOther, d.q16_other, "section-q16", "10. 請填寫其他環境設施", s4);

        v.require(d.q17_issue, "section-q17", "11. 注意事項未填寫", s4);
        v.requireIf(d.q17_issue === '是', d.q17_desc, "section-q17", "11. 請填寫注意事項說明", s4);
    }
    ...
    // --- LAND VALIDATION ---
        ...
        v.requireIf(d.land_q1_elec === '其他', d.land_q1_elec_other, "section-land-q1", "1. 請填寫電力「其他」說明", s2);
        ...
        v.requireIf(d.land_q1_water === '其他', d.land_q1_water_other, "section-land-q1", "1. 請填寫水源「其他」說明", s2);

        // Added Water Booster Validation
        v.require(d.water_booster, "section-land-q1", "1. 是否設置用戶加壓受水設備未填寫", s2);
        v.requireIf(d.water_booster === '其他', d.water_booster_desc, "section-land-q1", "1. 請填寫加壓設備「其他」說明", s2);

        // Old Q2 moved to Step 4, so here we validate old Q3 (now Q2)
        v.require(d.land_q3_survey, "section-land-q3", "2. 鑑界紀錄未填寫", s2);
        v.require(d.land_q3_dispute, "section-land-q3", "2. 糾紛情況未填寫", s2);

        // Old Q4 (now Q3)
        v.require(d.land_q4_expro, "section-land-q4", "3. 徵收預定地情況未填寫", s2);
        v.require(d.land_q4_resurvey, "section-land-q4", "3. 重測區域情況未填寫", s2);

        // Old Q5 (now Q4 in Step 2)
        v.require(d.land_q5_encroached, "section-land-q5", "4. 被占用情況未填寫", s2);
        v.requireIf(d.land_q5_encroached === '是', d.land_q5_encroached_desc, "section-land-q5", "4. 請填寫被占用說明", s2);
        
        v.require(d.land_q5_encroaching, "section-land-q5", "4. 占用鄰地情況未填寫", s2);
        v.requireIf(d.land_q5_encroaching === '是', d.land_q5_encroaching_desc, "section-land-q5", "4. 請填寫占用鄰地說明", s2);

        const s3 = 3;

        // Old Q6 (now Q5)
        v.require(d.land_q6_limit, "section-land-q6", "5. 禁建/限建情況未填寫", s3);
        ...
        v.requireIf(d.land_q7_user === '非所有權人使用', d.land_q7_user_detail, "section-land-q7", "6. 請選擇非所有權人使用情況", s3);

        v.require(d.land_q7_crops, "section-land-q7", "6. 農作物情況未填寫", s3);
        v.require(d.land_q7_build, "section-land-q7", "6. 地上建物情況未填寫", s3);
        v.require(d.land_q7_solar, "section-land-q7", "6. 太陽能光電發電設備情況未填寫", s3);
        
        const s4 = 4;
        // New Q7 (Old Q2) - Access
        v.require(d.land_q2_access, "section-land-q2", "7. 土地進出通行未填寫", s4);
        if (d.land_q2_access === '正常 (可自由通行)') {
             v.require(d.land_q2_owner, "section-land-q2", "7. 請選擇臨路歸屬權", s4);
             v.requireIf(d.land_q2_owner === '私人', d.land_q2_owner_desc, "section-land-q2", "7. 請填寫私人歸屬說明", s4);
        } else if (d.land_q2_access === '異常 (有阻礙)') {
             v.require(d.land_q2_access_desc, "section-land-q2", "7. 請填寫異常說明", s4);
        }

        // Soil
        v.require(d.soil_q1_status, "section-soil", "8. 土壤與地下埋設物未填寫", s4);
        v.requireIf(d.soil_q1_status === '有', d.soil_q1_desc, "section-soil", "8. 請填寫土壤/地下物說明", s4);

        const noEnv = d.q16_noFacilities === false && (d.q16_items?.length || 0) === 0 && !d.q16_hasOther;
        v.check(noEnv, "section-q16", "9. 環境設施未勾選", s4);
        
        v.require(d.land_q8_special, "section-land-q8", "10. 注意事項未填寫", s4);
        v.requireIf(d.land_q8_special === '是', d.land_q8_special_desc, "section-land-q8", "10. 請填寫注意事項說明", s4);
    }
    ...
    // --- FACTORY VALIDATION ---
        ...
        // Factory Utilities
        v.require(d.land_q1_elec, "section-factory-q6", "6. 電、水與其他設施使用情況未填寫 (電力)", s3);
        v.require(d.land_q1_water, "section-factory-q6", "6. 電、水與其他設施使用情況未填寫 (水源)", s3);

        // Added Water Booster Validation
        v.require(d.water_booster, "section-factory-q6", "6. 是否設置用戶加壓受水設備未填寫", s3);
        v.requireIf(d.water_booster === '其他', d.water_booster_desc, "section-factory-q6", "6. 請填寫加壓設備「其他」說明", s3);
        ...
        const simpleParking = ['透天獨棟廠房', '倉儲物流廠房', '其他'].includes(d.propertyType);
        if (simpleParking) {
        ...
        // Q10 Facilities (New)
        v.require(d.q9_hasIssue, "section-factory-q10", "10. 本案或本社區是否有須注意的設施未填寫", s3);
        if (d.q9_hasIssue === '是') {
            ...
            // Solar PV Maintenance Validation
            if (d.q9_items?.includes("太陽能光電發電設備")) {
                const label = ['透天獨棟廠房', '倉儲物流廠房'].includes(d.propertyType) ? "設置現況" : "維護方式";
        ...
        // Phase 2: Factory High-Rise Logic
        const isHiRise = (d.propertyType === "立體化廠辦大樓"); 
        const hideLandDetails = (d.propertyType === "立體化廠辦大樓" || d.propertyType === "標準廠房(工業園區內)");
        ...
        v.require(d.land_q2_access, "section-land-q2", `${accessNum}. 進出通行情況未填寫`, s4);
        
        if (!hideLandDetails) {
             v.require(d.land_q3_survey, "section-land-q3", `${landQ3Num}. 鑑界紀錄未填寫`, s4);
             v.require(d.land_q3_dispute, "section-land-q3", `${landQ3Num}. 糾紛情況未填寫`, s4);
             v.require(d.land_q4_expro, "section-land-q4", `${landQ4Num}. 徵收地預定地情況未填寫`, s4);
             v.require(d.land_q4_resurvey, "section-land-q4", `${landQ4Num}. 重測區域情況未填寫`, s4);
        }

        // Numbering adjusts based on hidden sections, but logic remains attached to keys
        // Soil - Validate if NOT hidden
        if (!hideSoil) {
             v.require(d.soil_q1_status, "section-soil", `${soilNum}. 土壤與地下埋設物未填寫`, s4);
             v.requireIf(d.soil_q1_status === '有', d.soil_q1_desc, "section-soil", `${soilNum}. 請填寫土壤/地下物說明`, s4);
        }
