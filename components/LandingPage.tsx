...
import { Home, Map as MapIcon, Car, Factory, LucideIcon } from 'lucide-react';
...
export const LandingPage: React.FC<LandingPageProps> = ({ onSelect }) => {
    return (
        ...
        <div className="h-full w-full bg-[#FDFBF7] overflow-y-auto overflow-x-hidden relative">
            ...
            <div className="fixed inset-0 overflow-hidden pointer-events-none">
                ...
                <div className="absolute top-0 right-0 w-64 h-64 bg-sky-100 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-2000"></div>
            </div>
            ...
            <div className="min-h-full flex flex-col items-center justify-center p-4 py-8 md:p-6 relative z-10">
                <div className="w-full max-w-xl flex flex-col gap-5 md:gap-8">
                    ...
                    <div className="bg-white/80 backdrop-blur-sm p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-xl border-4 border-white ring-4 ring-slate-100 text-center space-y-3 md:space-y-4 animate-in slide-in-from-bottom-4 duration-700">
                        <h1 className="text-3xl md:text-5xl font-black text-slate-800 tracking-tight">
                            幸福家不動產
                        </h1>
                        <div className="w-16 md:w-24 h-2 bg-orange-400 mx-auto rounded-full"></div>
                        <p className="text-slate-500 font-bold text-xl md:text-2xl tracking-widest">
                            物件現況調查系統
                        </p>
                    </div>
                    ...
                    <div className="grid gap-4 md:gap-5 animate-in slide-in-from-bottom-8 duration-700 delay-150">
                        {OPTIONS.map((opt) => (
                            <button 
                                ...
                                className={`group relative w-full py-5 md:py-6 px-6 md:px-8 rounded-[1.5rem] md:rounded-[2rem] ${opt.bg} text-white transition-all duration-200 
                                border-4 border-transparent border-b-[6px] md:border-b-[8px] ${opt.border} 
                                hover:brightness-110 active:scale-[0.98] active:border-b-4 active:translate-y-[4px]
                                flex items-center justify-between shadow-xl ${opt.shadow}`}
                            >
                                <div className="flex items-center gap-4 md:gap-6">
                                    <div className="bg-white/20 p-3 md:p-4 rounded-xl md:rounded-2xl backdrop-blur-md group-hover:scale-110 transition-transform duration-300">
                                        <opt.icon className="w-8 h-8 md:w-12 md:h-12 text-white" strokeWidth={3} />
                                    </div>
                                    <span className="text-2xl md:text-4xl font-black tracking-wide drop-shadow-md">
                                        {opt.label}
                                    </span>
                                </div>
                                <div className="bg-white/20 rounded-full p-1.5 md:p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <div className="text-center mt-2 md:mt-0">
                        <p className="text-slate-400 font-bold text-xs md:text-sm tracking-wider uppercase">
